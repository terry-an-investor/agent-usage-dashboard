...
- **Node.js**（含 npm）—— 安装依赖 `ccusage` 需要它，`refresh.sh` 也依赖 node 运行
- **Python 3** —— 本地服务 `serve.py`（仅标准库，无需 pip 安装任何东西）
- 本机装过 **Kimi Code** 且有过会话记录（数据来自 `~/.kimi-code/sessions/**/wire.jsonl`）

## 快速开始

```bash
npm install   # 安装依赖 ccusage（数据聚合用，固定本地版本 + 离线模式，不联网查 registry）
./start.sh    # 启动本地服务（127.0.0.1:8931）并打开页面
```

> 如果跳过 `npm install` 直接运行，`refresh.sh` 会因找不到 `node_modules/.bin/ccusage` 而报错。
